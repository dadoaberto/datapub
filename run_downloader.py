...
def run_downloader(orgao, args):
    ...
    if orgao == "al_go":
        ...
        print(f"🚀 Iniciando download AL-GO de {params['start_date']} a {params['end_date']}")
    
    elif orgao == "al_ms":
        params["start_num"] = int(args.start) if args.start else 1844
        params["end_num"] = int(args.end) if args.end else None
        print(f"🚀 Iniciando download AL-MS do número {params['start_num']} até {'último disponível' if not params['end_num'] else params['end_num']}")
    
    elif orgao == "estado_sp":
...
def main():
    ...
    # Configuração para Estado SP
    parser_sp = subparsers.add_parser("estado_sp", help="Diários de São Paulo")
